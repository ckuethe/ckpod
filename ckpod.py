...
def parse_args():
    ...
    parser.add_argument('-c', '--confdir', dest='confdir',
                        default=os.path.expanduser('~/.ckpod'), type=str,
                        help='path of the configuration directory')
    parser.add_argument('-d', '--downloads', dest='downloads',
                        default=4, type=int,
...
def ensure_config(args):
    ...
    for name in CKPOD_CONFIG.sections():  # hack to allows "%(name)" interpolations on python2
...
def download_episode_list(dl_args):
    ...
    if not resp.ok:
        logging.debug("%s: HTTP/%d while downloading %s", dl_args[0], resp.status_code, resp.url)
        return (dl_args[0], False)
    ...
    for episode in pod.items:
        ...
        try:
            pubtime = arrow.get(episode.published_date,
                                ["D MMM YYYY HH:mm:ss", "D MMMM YYYY HH:mm:ss"]).datetime
        except arrow.parser.ParserError:
            logging.warning("%s: unable to parse episode publish date '%s'. Using current time.",
                            dl_args[0],
                            episode.published_date)
            pubtime = arrow.get().datetime

        episodes.append((dl_args[0],
...
def download_episode(episode):
    ...
    if CKPOD_CONFIG[podname]['sed']:
        sed_search, sed_replace, _ = re.match(r'''^s(.)(.+?)\1(.+?)\1(.+?)?$''',
                                              CKPOD_CONFIG[podname]['sed']).groups()[1:4]
        new_name = re.sub(sed_search, sed_replace, episode['url'])
        params['remote_name'] = new_name

    disk_file_name = os.path.expanduser(os.path.join(CKPOD_CONFIG[podname]['destdir'],
                                        params['remote_name']))
    ...
    resp = None
    try:
        resp = requests.get(episode['url'], stream=True, headers=resume_header, timeout=10)
...
def main():
    ...
    feeds = filter(lambda section_name: section_name not in ['example', 'DEFAULT'],
                   CKPOD_CONFIG.sections())
    feed_urls = map(lambda feed_name: (feed_name, CKPOD_CONFIG[feed_name]['url']), feeds)

    workers = ThreadPool(program_args.downloads)
    logging.info("Refreshing %d feeds with %d threads", len(feed_urls), program_args.downloads)
    ...
    logging.info("Downloading %d episodes with %d threads", len(jobs), program_args.downloads)
